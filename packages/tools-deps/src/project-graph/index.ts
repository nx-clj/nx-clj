...
export function processProjectGraph(
  ...
  for (const projectName in context.filesToProcess) {
    const depsEdnFiles = context.filesToProcess[projectName].filter(
      ({ file }) => basename(file) === 'deps.edn'
    );

    const dependencies = depsEdnFiles.map(({ file }) => ({
      file,
      dependencies: getDependencyPathsFromDepsEdn(file)
        .map((relPath) => join(graph.nodes[projectName].data.root, relPath))
        .map((path) => projectRootLookupMap.get(path)),
    }));

    dependencies.forEach(({ dependencies, file }) =>
...
function getDependencyPathsFromDepsEdn(filePath: string): string[] {
  const content = parseEDNString(readFileSync(filePath).toString());
